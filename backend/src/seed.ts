...
import { initDatabase, get, run } from './db.js';

async function seed() {
  await initDatabase();

  const userRow = await get<{ count: number }>('SELECT COUNT(*) as count FROM users');
  ...
  await run(
    ...
    ['admin', password, '系统管理员', 'admin', '13800000000']
  );

  const c1 = await run(
    'INSERT INTO communities (name, address, area, contact_phone, description) VALUES (?, ?, ?, ?, ?)',
    ['阳光花园', '北京市朝阳区阳光路88号', 120000, '010-88888888', '高端住宅小区，绿化率40%']
  );
  const c2 = await run(
    'INSERT INTO communities (name, address, area, contact_phone, description) VALUES (?, ?, ?, ?, ?)',
    ['翠湖名苑', '北京市海淀区翠湖路66号', 85000, '010-66666666', '毗邻翠湖公园，环境优美']
  );
  ...
  for (const b of buildings) {
    const result = await run(
      ...
      [b.community_id, b.name, b.floors, b.units_per_floor]
    );
    ...
    for (let f = 1; f <= Math.min(b.floors, 5); f++) {
      for (let u = 1; u <= b.units_per_floor; u++) {
        ...
        const result = await run(
          ...
          [buildingIds[bi], unitNumber, f, area, type, status]
        );
  ...
  for (let i = 0; i < 10; i++) {
    ...
    await run(
      ...
      [unitId, residentNames[i], residentPhones[i],
  ...
  for (const ft of feeTypes) {
    const result = await run(
      ...
      [ft.name, ft.unit_price, ft.unit, ft.description]
    );
    feeTypeIds.push(result.insertId);
  }
  ...
  for (let i = 0; i < 20; i++) {
    ...
    const unit = await get<{ area: number }>('SELECT area FROM units WHERE id = ?', [unitId]);
    const amount = feeTypeId === feeTypeIds[0]
      ? Math.round((unit?.area || 0) * 2.5 * 100) / 100
    ...
    await run(
      ...
      [unitId, feeTypeId, amount, periods[i % periods.length], status, '2026-01-31', paidAt]
    );
  }
  ...
  for (let i = 0; i < 8; i++) {
    ...
    await run(
      ...
      [unitIds[i], i + 1, repairTitles[i], `住户报修：${repairTitles[i]}，请尽快处理。`,
  ...
  for (const a of announcements) {
    await run(
      ...
      [a.title, a.content, a.type, '物业管理处', a.is_pinned]
    );
  }
  ...
  for (let i = 1; i <= 30; i++) {
    ...
    await run(
      ...
      [communityId, `${i <= 15 ? 'A' : 'B'}-${String(i % 15 + 1).padStart(3, '0')}`,
